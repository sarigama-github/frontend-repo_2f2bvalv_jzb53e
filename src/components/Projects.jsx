      ...
      {tags.map(tag => {
        ...
        return (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs border transition ${isActive ? 'bg-white text-slate-900 border-white' : 'bg-white/5 text-blue-200/80 border-white/10 hover:bg-white/10'}`}
          >
            {tag}
          </button>
        )
    ...
    <section id="projects" className="relative py-20">
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        <div className="flex items-end justify-between flex-wrap gap-4 mb-8">
          ...
          {allTags.length > 0 && (
            <TagFilters tags={allTags} active={activeTags} onToggle={toggleTag} />
          )}
        </div>
